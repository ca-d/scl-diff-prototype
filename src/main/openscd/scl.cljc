(ns openscd.scl
  (:require [openscd.schema :as schema]))
...
(def scl
  (memoize (fn [{:keys [tag attrs content], :as xml} root elements]
             (if (string? xml)
               xml
               (let [transform (get spe-scl tag (fn [x _ _] x))]
                 (transform {:tag tag,
                             :attrs attrs,
                             :content (if content
...
(defn ^:export domToEdn
  [dom]
  (condp = (.-nodeType dom)
    3 (.-textContent dom) ; text
    9 (recur (.-documentElement dom)) ; document
    1 {:tag (.-tagName dom),
       :attrs (apply cljs.core/array-map
                (mapcat (fn [a] [(keyword (.-name a)) (.-value a)])
                  (.-attributes dom))),
       :content (filter (complement nil?) (map domToEdn (.-childNodes dom)))} ; element
    nil))
