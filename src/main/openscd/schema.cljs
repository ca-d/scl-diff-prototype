...
(def scl-schema-text
  "XML schema for Substation Configuration Language (SCL) as defined in IEC 61850-6."
...
xmlns:dean=\"http://www.w3.org/2001/XMLSchema\" xmlns:xs=\"http://www.w3.org/2001/XMLSchema\">
<xs:annotation>
...
(def scl-schema
  (.parseFromString (js/DOMParser.) scl-schema-text "application/xml"))

(def attributes
  (group-by :name
            (map #(into {}
                        (map (fn [attr] [(keyword (.-name attr))
                                         (.-value attr)])
                          (.-attributes %)))
              (.getElementsByTagName scl-schema "xs:attribute"))))

(def attribute-counts (into {} (map (fn [[k v]] [k (count v)]) attributes)))

(defn conflicting-defaults?
  [entry]
  (->> entry
       val
       (map :default)
...
(def problematic-attributes
  (into {}
        (map (fn [[k v]] [k (count (filter identity (map :default v)))])
          (filter conflicting-defaults? attributes))))

(def unproblematic-attributes
  (into {} (filter (complement conflicting-defaults?) attributes)))

(def default-counts
  (into {}
        (map (fn [[k v]] [k (count (filter #(contains? % :default) v))])
          attributes)))
